...
import type { PARQuestion } from "@/data/par-questions";

type AnswerMap = Record<string, string>;

export default function PARTestTakePage() {
  return (
    <Suspense fallback={
...
function PARTestTakeInner() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const isMini = searchParams.get("mini") === "true";
  const idsParam = searchParams.get("ids") || "";

  const [questions, setQuestions] = useState<PARQuestion[]>([]);
  ...
  const fetchQuestions = useCallback(async () => {
    try {
      if (isMini && idsParam) {
        // Mini-test: fetch specific questions by fetching all and filtering
        const ids = idsParam.split(",");
        const res = await fetch("/api/par/questions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ aoks: [], count: 100 }),
        });
        const data = await res.json();
        const all: PARQuestion[] = data.questions || [];
        const filtered = ids
          .map((id) => all.find((q) => q.id === id))
          .filter(Boolean) as PARQuestion[];
        setQuestions(filtered.length > 0 ? filtered : all.slice(0, 20));
      } else {
        const res = await fetch("/api/par/questions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ aoks: [], count: 60 }),
        });
        if (!res.ok) {
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await fetch("/api/par/test/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (isMini) {
        // For mini-tests, show inline results summary
        router.push(
          `/par-practice/session?aoks=&count=0&mini=done&score=${data.score}&total=${data.total}`
        );
        return;
      }

      if (data.attemptId) {
        router.push(`/par-test/results/${data.attemptId}`);
      } else {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-sm font-semibold">
            {isMini ? "Mini-Test" : "PAR Sample Test"}
            {isMini && (
