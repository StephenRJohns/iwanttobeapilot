...
export default function TestHubClient({ bank, examQuestionCount, passCount }: TestHubClientProps) {
  ...
  useEffect(() => {
    if (tab === "history" && history.length === 0) {
      ...
      fetch(`/api/test-bank/${bankLower}/history`)
        .then((r) => r.json())
    ...
    if (tab === "weak" && weak.length === 0) {
      setWeakLoading(true);
      fetch(`/api/test-bank/${bankLower}/weak-answers`)
        .then((r) => r.json())
        .then((d) => setWeak(d.weakAnswers || []))
        .finally(() => setWeakLoading(false));
    }
  }, [tab, history.length, weak.length, bankLower]);

  async function handleStartTest() {
    setStartingTest(true);
    router.push(`/test-prep/${bankLower}/take`);
  }

  function handleMiniTest() {
    const ids = weak.map((w) => w.questionId).join(",");
    router.push(`/test-prep/${bankLower}/take?mini=true&ids=${ids}`);
  }
        ...
        <div className="max-w-lg">
          <div className="rounded-lg border border-border bg-card p-6 text-center mb-4">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              {examQuestionCount} random questions from all areas of knowledge. You need{" "}
              <span className="font-semibold text-foreground">
                70% ({passCount}/{examQuestionCount})
              </span>{" "}
              to pass — the same threshold as the real FAA knowledge test.
            </p>
            <button
            ...
            >
              ...
              Generate {examQuestionCount}-Question Test
            </button>
          </div>
                  ...
                  {history.map((attempt) => {
                    ...
                    return (
                      <tr key={attempt.id} className="hover:bg-card/50">
                        ...
                        <td className="py-3">
                          <Link
                            href={`/test-prep/${bankLower}/results/${attempt.id}`}
